...
export function Index() {
  ...
  const [jenisBarang, setJenisBarang] = useState({data: []});
  ...
  useEffect(() => {
    document.title = "Jenis Barang";
    axios.defaults.headers.common["Authorization"] = `Bearer ${token}`;
    axios.get(`${appConfig.baseurlAPI}/jenis_barang`, {params: values}).then((response) => {
  ...
  const handleDelete = (e) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`${appConfig.baseurlAPI}/jenis_barang/${e}`).then((response) => {
          Swal.fire(
            'Deleted!',
      ...
      <section className="section">
        ...
        <div className="col-12">
          <div className="card">
                ...
                <table className="table table-striped">
                  <thead>
                    <tr>
                      ...
                      <th>Deskripsi <ButtonSortir handleSearchChange={handleSearchChange} values={values} name="deskripsi"/></th>
                      <th>Action</th>
                    </tr>
                  </thead>
                  <tbody>
                    {jenisBarang.data.map((jenis, index) => {
                      return (
                        <tr key={index}>
                          <td>{ jenis.nama }</td>
            ...
            <Pagination links={jenisBarang.links} handleSearchChange={handleSearchChange}/>
          </div>
        </div>
      </section>
    </>
